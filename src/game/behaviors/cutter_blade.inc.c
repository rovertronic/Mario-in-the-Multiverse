   ...
   struct Surface *surf;
        ...
        if (rayResult > 0) {
            if (o->oAction == 0) {
                ...
                o->oAction = 3;
                if (rayResult == 2) o->oVelY = 10.0f;
                if (rayResult == 3) o->oVelY = -10.0f;
                o->oAngleVelYaw = 0x800;
                o->oAngleVelPitch = random_u16() / 64;
                o->oAngleVelRoll = random_u16() / 64;
                o->oForwardVel *= -0.05f;
   ...
   obj_set_hitbox(o, &sCutterBladeHitbox);
    ...
    for (int i = 0; i < o->numCollidedObjs; i++) {
        ...
        if (other != gMarioObject) {
            ...
            spawn_object_relative(0, 0, 100, 0, gMarioObject, MODEL_NONE, bhvSparkleSpawn);
            obj_mark_for_deletion(o);
        }
    }
}
